...
- [aws-sam.md](aws-sam.md) — **before touching `server/` templates or the build.** AWS access is CLI/boto3, not an MCP tool (dev vs. prod stacks and accounts), and the AWS CLI is absent by default on the cloud runner (declare it in the environment setup script). (Portable subsets — the deploy role's transform + CloudFront grants, the new-account CloudFront-verification gate, and failed-`CREATE` cleanup — now live in the canon `aws-sam` pack.)

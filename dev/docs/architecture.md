...
└── .github/workflows/               # test-server.yml, test-client.yml, test-requirements.yml, deploy.yml, chrome-extension-release.yml (+ aws-status.yml)
```

### 8.2 Build artifacts and checks from one repo (CI)
🔧 Path-filtered **`test-server.yml`** and **`test-client.yml`** run build+test independently (each also runs the
...
- **CI split into `test-server.yml`/`test-client.yml` + separate `deploy.yml` and the single `chrome-extension-release.yml`**

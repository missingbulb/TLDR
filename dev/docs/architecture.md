...
└── .github/workflows/               # server.yml, client.yml, requirements.yml, deploy.yml, chrome-extension-release.yml (+ aws-status.yml)
```
...
- **CI split into `server.yml`/`client.yml` + separate `deploy.yml` and the single `chrome-extension-release.yml`**

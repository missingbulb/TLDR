...
## The category-wiring risk belongs in `dev/requirements/`, not in a pack

The weekly `growth-discover-packs` run authored a `tldr-categories` local pack (PR #121, closed
unmerged) whose blocking checks asserted that every id in `shared/categories.mjs` carries its design
descriptor, its scoped stylesheet, its registry entry and its `sidepanel.html` link. The
silent-failure risk it named is real and still unhomed: its home is `dev/requirements/` — one
numbered leaf claimed by one case — and the extension's own suite. Don't re-answer it with a pack.
